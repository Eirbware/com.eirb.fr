<script lang="ts">
	...
	import { getChatId, setChatId, setDraft } from './store-inputs';
	import { botPublicCredentials } from '$lib/bot-public-credentials';
	...
	function messageSubmit(event: SubmitEvent) {
		...
		postMessage($state.snapshot(htmlRenderedContent()), $state.snapshot(chatId));
	}
...
> The previewed message will be sent to you by [\`${botPublicCredentials.username}\`](${botPublicCredentials.link}). Contact him in private on Telegram, next run the \`/start\` command to get your chat id. Then, fill the field below with this chat id and click on *Submit* to get the message in your private Telegram chat.
...
`;
	onMount(() => {
		if ($state.snapshot(markdownContent.content) === '') markdownContent.content = defaultText;
	});
</script>
...
The previewed message will be sent to you by {botPublicCredentials.username}. Contact him in private on Telegram, next run the '/start' command to get your chat id. Then, fill the field below with this chat id and click on Submit to get the message in your private Telegram chat.
...
"
	...
	<div class="input-row">
		...
		<button form="noform" onclick={openHelp} id="help-button" title="Où trouver le chat-id ?">
			<img
