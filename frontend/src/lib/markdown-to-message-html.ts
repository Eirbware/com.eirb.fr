...
const underlineProcessing: MarkdownRenderingCustomProcess = {
	pre: (mdContent) =>
		mdContent.replace(/\+/g, '%u%').replace(/<u>/g, '++').replace(/<\/u>/g, '++%%verydeadbeef%%'),
	post: (htmlContent) =>
		htmlContent
...
const taskListProcessing: MarkdownRenderingCustomProcess = {
	pre: (mdContent) => mdContent,
	post: (htmlContent) => {
		const $ = cheerio.load(htmlContent);
    $('input.task-list-item-checkbox[checked=""]').replaceWith('✅');
    $('input.task-list-item-checkbox').replaceWith('🔲');
    return $.html();
	}
};
...
const pipe = (ps: ((s: string) => string)[]) => (s: string) => ps.reduce((acc, fn) => fn(acc), s);

const preprocesses = pipe(processes.map((p) => p.pre));
const postprocesses = pipe(processes.reverse().map((p) => p.post));

export const renderMarkdown = pipe([preprocesses, (s) => md.render(s), postprocesses]);
